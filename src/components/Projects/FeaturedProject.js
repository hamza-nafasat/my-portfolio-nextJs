  ...
  return (
    ...
    >
      ...
      <FeaturedImage
        src={image}
        alt={title}
        className="w-1/2 cursor-pointer overflow-hidden rounded-lg lg:w-full h-auto "
        whileTap={{ scale: 1 }}
      ...
      <div className="w-1/2 flex flex-col items-start justify-between pl-6 lg:w-full lg:pl-0 lg:pt-6">
        ...
        {/* </Link> */}
        {/* SUMMERY  */}
        <p className="my-2 font-medium text-dark dark:text-light md:text-sm xl:text-sm lg:text-base ">{summery}</p>
        {/* PROJECT LINKS  */}
        {/* ============== */}
        <div className="flex mt-2 items-center">
          <MotionLink
            href={github}
            target="_blank"
            className={`w-10 ${!github && "cursor-not-allowed opacity-30"}`}
            whileTap={{ scale: 0.9 }}
            transition={{ duration: 0.3 }}
          ...
          <Link
            ...
            className={`ml-4 rounded-lg bg-dark text-light hover:bg-light hover:text-dark
						...
						sm:text-sm hover:dark:text-light sm:px-4 ${!link && "cursor-not-allowed opacity-50 pointer-events-none"}`}
          >
            Visit Project
          </Link>
        </div>
